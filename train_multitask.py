"""
Multi-task training pipeline for satellite keypoint pose estimation.

Two outputs from a shared MobileNetV2 backbone:
  1. Heatmaps (one per keypoint) -> localizes each keypoint in the image
  2. Visibility logits (one per keypoint) -> predicts visible vs occluded/out-of-bounds
...
heatmap_dir/
    img000001_point00.npy   # (H, W) float array, Gaussian heatmap for keypoint 0
    img000001_point01.npy
    ...
    img000001_point10.npy   # up to num_keypoints - 1
    img000002_point00.npy
    ...

visibility_json (single file):
    {
      "img000004.jpg": [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2],   # 2 = visible, 1 = occluded/out of bounds
      "img000005.jpg": [...],
      ...
    }

All key hyperparameters are exposed via the Config dataclass below and can be
overridden from the command line, e.g.:

    python train_satellite_pose.py \
        --image_dir /data/images --heatmap_dir /data/heatmaps \
        --visibility_json /data/visibility.json \
        --batch_size 32 --num_epochs 60 --freeze_epochs 6 \
        --lr_frozen 1e-3 --lr_finetune 1e-4 --vis_loss_weight 0.1
"""
...
class Config:
    ...
    # --- resuming ---
    resume: bool = True        # if True, resume from checkpoint_dir/last_checkpoint.pt if it exists
...
# ============================================================
# 3. Model: MobileNetV2 backbone + heatmap head + visibility head
# ============================================================
class MobileNetMultiTaskNet(nn.Module):
    ...
    def forward(self, x):
        ...
        vis_logits = self.vis_head(v)  # (B, K)
...
def compute_total_loss(pred_heatmaps, target_heatmaps, pred_vis_logits, target_visibility, vis_loss_weight):
    hm_loss = masked_heatmap_loss(pred_heatmaps, target_heatmaps, target_visibility)
    vis_loss = nn.functional.binary_cross_entropy_with_logits(pred_vis_logits, target_visibility)
    total = hm_loss + vis_loss_weight * vis_loss
    return total, hm_loss.item(), vis_loss.item()


# ============================================================
# 5. Validation metrics
#    - both loss terms, reported separately
#    - precision / recall / F1 / accuracy for the visibility classifier
#      (aggregate, plus a per-keypoint breakdown)
...
def binary_classification_metrics(pred, target, eps=1e-8):
    """
    pred, target: same-shape binary (0/1) tensors, any shape (will be flattened).
    Returns precision, recall, f1, accuracy as plain floats.
    """
    pred = pred.reshape(-1)
...
@torch.no_grad()
def evaluate(model, loader, device, vis_loss_weight, num_keypoints):
    model.eval()
    total_loss, total_hm, total_vis = 0.0, 0.0, 0.0
    pck_thresholds = (2.0, 5.0, 10.0, 20.0)
    pck_correct = {threshold: 0 for threshold in pck_thresholds}
    pck_total = 0
    pck_all_correct = {threshold: 0 for threshold in pck_thresholds}
    pck_all_total = 0
    ...
    for images, target_hm, target_vis in loader:
        ...
        loss, hm_l, vis_l = compute_total_loss(pred_hm, target_hm, pred_vis_logits, target_vis, vis_loss_weight)
        ...
        visible_keypoints = mask.bool()
        pck_total += visible_keypoints.sum().item()
        pck_all_total += pixel_dist.numel()
        for threshold in pck_thresholds:
            pck_correct[threshold] += (
                (pixel_dist <= threshold) & visible_keypoints
            ).sum().item()
            pck_all_correct[threshold] += (pixel_dist <= threshold).sum().item()
        ...
        pred_vis_bin = (torch.sigmoid(pred_vis_logits) > 0.5).float()
        all_pred_vis.append(pred_vis_bin.cpu())
        all_target_vis.append(target_vis.cpu())

    n = len(loader)
    all_pred_vis = torch.cat(all_pred_vis, dim=0)      # (N, K)
    all_target_vis = torch.cat(all_target_vis, dim=0)  # (N, K)

    aggregate_vis_metrics = binary_classification_metrics(all_pred_vis, all_target_vis)

    per_kp_pixel_error = (per_kp_pixel_error_sum / per_kp_visible_count.clamp(min=1)).cpu().tolist()
    per_kp_vis_metrics = [
        binary_classification_metrics(all_pred_vis[:, k], all_target_vis[:, k])
        for k in range(num_keypoints)
    ]
    ...
    return {
        ...
        "val_vis_loss": total_vis / n,

        "mean_pixel_error": sum(per_kp_pixel_error) / len(per_kp_pixel_error),
        "per_kp_pixel_error": per_kp_pixel_error,
        "pck": {
            threshold: correct / max(pck_total, 1)
            for threshold, correct in pck_correct.items()
        },
        "pck_all": {
            threshold: correct / max(pck_all_total, 1)
            for threshold, correct in pck_all_correct.items()
        },

        "vis_accuracy": aggregate_vis_metrics["accuracy"],
        "vis_precision": aggregate_vis_metrics["precision"],
        "vis_recall": aggregate_vis_metrics["recall"],
        "vis_f1": aggregate_vis_metrics["f1"],
        "vis_confusion": {k: aggregate_vis_metrics[k] for k in ("tp", "fp", "fn", "tn")},
        "per_kp_vis_metrics": per_kp_vis_metrics,
    ...
    try:
        for epoch in range(start_epoch, cfg.num_epochs):
            ...
            for images, target_hm, target_vis in train_loader:
                ...
                optimizer.zero_grad()
                pred_hm, pred_vis_logits = model(images)
                loss, hm_l, vis_l = compute_total_loss(
                    pred_hm, target_hm, pred_vis_logits, target_vis, cfg.vis_loss_weight
                )
                loss.backward()
                optimizer.step()
            ...
            print(f"           mean_pixel_error={val_metrics['mean_pixel_error']:.2f}px | "
                f"PCK@2={val_metrics['pck'][2.0]:.4f} "
                f"PCK@2(all)={val_metrics['pck_all'][2.0]:.4f} "
                f"PCK@5={val_metrics['pck'][5.0]:.4f} "
                f"PCK@10={val_metrics['pck'][10.0]:.4f} "
                f"PCK@20={val_metrics['pck'][20.0]:.4f} | "
...
#     [finetune] epoch 36/36 | train_loss=0.00198 (hm=0.00178, vis=0.39583)
#            val_loss=0.00237 (hm=0.00217, vis=0.41060)
#            mean_pixel_error=0.15px | vis_acc=0.8740 precision=0.8740 recall=1.0000 f1=0.9328 (tp=17305 fp=2495 fn=0 tn=0)
#            mean_heatmap_peak: pred=0.7826 vs target=0.9966 
#            mean_keypoint_spread_ratio=1.2667 (min over batch=0.6114, 1.0=healthy, near 0=collapsed)
#            -> saved new best checkpoint to checkpoints\best_model.pt
# Training finished. Best val_loss=0.00237


# [finetune] epoch 39/39 | train_loss=0.00224 (hm=0.00204, vis=0.39634)
#            val_loss=0.00273 (hm=0.00253, vis=0.39657)
#            mean_pixel_error=0.17px | PCK@2=0.9967 PCK@2(all)=0.9956 PCK@5=0.9999 PCK@10=1.0000 PCK@20=1.0000 | vis_acc=0.8740 precision=0.8740 recall=1.0000 f1=0.9328 (tp=17305 fp=2495 fn=0 tn=0)
#            mean_heatmap_peak: pred=0.7747 vs target=0.9966 
#            mean_keypoint_spread_ratio=1.3664 (min over batch=0.4890, 1.0=healthy, near 0=collapsed)
# Training finished. Best val_loss=0.00237